...
module.exports = {
  ...
  getHotelById: catchAsync(async (req, res) => {
    const hotel = await HotelsService.getById(req.params.id);
    if (!hotel) {
      throw errorsUtil.createNotFound(`Can't find hotel with id ${req.params.id}`);
    }
    return res.status(200).json({
      status: HttpStatusEnum.Success,
      statusCode: HttpStatusCodeEnum.Ok,
  ...
  createHotel: catchAsync(async (req, res) => {
    ...
    if (imageFiles.length) {
      const hotelImagePayloads = constructFilePayloads(req, imageFiles);
